var map, pointarray, heatmap;
	...
	function update_table_pol_change(){
		...
		$.ajax({url:"/mexbalia/Geo_Agg/UI/Stores_Coords.xsjs",data:{polygon:get_polygon(),categories:cadena_cat},success:function(respuesta){
	...
	function update_table(){
		...
		if((markersArray_store.length!=0)&&(markersArray.length==0)){
			var lat_1 = map.getBounds().getSouthWest().k;
			var lon_1 = map.getBounds().getSouthWest().A;
			var lat_2 = map.getBounds().getNorthEast().k;
			var lon_2 = map.getBounds().getNorthEast().A;
	...
	poligono.setPath([]);
		...
		$.ajax({url:"/mexbalia/Geo_Agg/UI/Stores_Coords.xsjs",data:{polygon:poligono_2,categories:cadena_cat},success:function(respuesta){
			var response = respuesta.map;
			insert_data(respuesta);
	...
	function placeMarker(location) {
		...
		google.maps.event.addListener(marker, 'click', function(event) {
			...
			if((event.latLng.A == first_mar_long)&&(event.latLng.k == first_mar_lat)&&(last_mark!=0)&&(poligono.getPath().length==0)){
				update_table_pol_change();
				
				
			}
			else if(((event.latLng.A == last_mar_long)&&(event.latLng.k == last_mar_lat))||last_mark==0){
...
poligono.setPath([new google.maps.LatLng(lat_1,lon_1),
                  ...
                  new google.maps.LatLng(lat_1,lon_2),
                  new google.maps.LatLng(lat_1,lon_1)]);
$.ajax({url:"/mexbalia/Geo_Agg/UI/Stores_Coords.xsjs",data:{polygon:get_polygon(),categories:cadena_cat},
